...
import { UserSettings, PromptTemplate } from '@/types';
...
const OptionsPage: React.FC = () => {
  ...
  const [newTemplate, setNewTemplate] = useState({ title: '', prompt: '', description: '' });

  useEffect(() => {
    const loadSettings = async () => {
      try {
        const result = await chrome.storage.local.get(['settings', 'defaultOptionsTab']);
        const s = result?.settings ? { ...defaultSettings, ...result.settings } : defaultSettings;
        setSettings(s);
        
        // Set the active tab if specified
        if (result?.defaultOptionsTab) {
          setActiveTab(result.defaultOptionsTab);
          // Clear the defaultOptionsTab after using it
          chrome.storage.local.remove(['defaultOptionsTab']);
        }
      } finally {
        setIsLoading(false);
      }
  ...
  const handleCreateNew = () => {
    ...
    setNewTemplate({ title: '', prompt: '', description: '' });
  };
  ...
  const handleAddTemplate = async () => {
    ...
    try {
      const template = await TemplateManager.addTemplate(newTemplate.title, newTemplate.prompt, newTemplate.description);
      const updatedTemplates = await TemplateManager.getTemplates();
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      await TemplateManager.updateTemplate(editingTemplate.id, {
  ...
  const handleDeleteTemplate = async (id: string) => {
    ...
    try {
      ...
      const updatedTemplates = await TemplateManager.getTemplates();
      setTemplates(updatedTemplates);
      // Select first template if current was deleted
  ...
  const handleResetTemplates = async () => {
    if (!confirm('Are you sure you want to reset all templates to defaults? This will delete all custom templates.')) return;
              ...
              <div className="h-[600px] w-full flex gap-6">
                ...
                <div className="w-1/3 flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-[#3d3929] dark:text-[#f0eee5]">Templates</h3>
                    <div className="flex gap-2">
                      <button
                        onClick={handleCreateNew}
                        className="px-3 py-2 bg-gradient-to-r from-[#da7756] to-[#bd5d3a] hover:from-[#bd5d3a] hover:to-[#a8462a] text-white rounded-lg font-medium transition-all duration-200 text-sm flex items-center gap-1"
                      >
                      ...
                      <button
                        ...
                        className="px-3 py-2 bg-gradient-to-r from-[#8a8470] to-[#6b6651] hover:from-[#6b6651] hover:to-[#3d3929] text-white rounded-lg font-medium transition-all duration-200 text-sm"
                      ...
                      <button
                        ...
                        className={`w-full text-left p-3 rounded-lg border transition-all duration-200 ${
                          selectedTemplate?.id === template.id
                            ? 'bg-[#da7756]/10 border-[#da7756]/30 shadow-sm'
                            : 'bg-[#f5f3ea]/80 dark:bg-[#342f28]/80 border-[#e8e4d5]/50 dark:border-[#4a453c]/50 hover:bg-[#da7756]/5'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex-1 min-w-0">
                            <h4 className={`font-medium text-sm truncate ${
                              selectedTemplate?.id === template.id
                                ? 'text-[#bd5d3a] dark:text-[#da7756]'
                                : 'text-[#3d3929] dark:text-[#f0eee5]'
                            }`}>
                              {template.title}
                            </h4>
                          ...
                          <div className="flex items-center gap-1 ml-2">
                            {template.isDefault && (
                              <span className="px-1.5 py-0.5 bg-[#da7756]/20 text-[#bd5d3a] dark:text-[#da7756] text-xs rounded font-medium">
                                Default
                              </span>
                            )}
                            {!template.isDefault && (
                ...
                <div className="flex-1 flex flex-col">
                  {isCreatingNew ? (
                    <div className="flex flex-col h-full">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-semibold text-[#3d3929] dark:text-[#f0eee5]">Create New Template</h3>
                        <button
                          onClick={handleCancelEdit}
                          className="p-2 text-[#8a8470] hover:text-[#da7756] transition-colors"
                        >
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <div className="space-y-4 flex-1 flex flex-col">
                        <div>
                          <label className="block text-sm font-semibold text-[#3d3929] dark:text-[#f0eee5] mb-2">Title *</label>
                          <input 
                            type="text"
                            value={newTemplate.title}
                            onChange={(e) => setNewTemplate(prev => ({ ...prev, title: e.target.value }))}
                            placeholder="Template title..."
                            className="w-full px-4 py-3 border border-[#d4d0c1] dark:border-[#4a453c] rounded-lg bg-[#eeece2] dark:bg-[#2a2520] text-[#3d3929] dark:text-[#f0eee5] focus:ring-2 focus:ring-[#da7756] focus:border-transparent transition-all duration-200"
                          />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-[#3d3929] dark:text-[#f0eee5] mb-2">Description</label>
                          <input 
                            type="text"
                        ...
                        <div className="flex-1 flex flex-col">
                          <label className="block text-sm font-semibold text-[#3d3929] dark:text-[#f0eee5] mb-2">Prompt *</label>
                          <textarea 
                            value={newTemplate.prompt}
                            onChange={(e) => setNewTemplate(prev => ({ ...prev, prompt: e.target.value }))}
                            placeholder="Enter the prompt that will be sent to the AI..."
                            className="flex-1 w-full px-4 py-3 border border-[#d4d0c1] dark:border-[#4a453c] rounded-lg bg-[#eeece2] dark:bg-[#2a2520] text-[#3d3929] dark:text-[#f0eee5] focus:ring-2 focus:ring-[#da7756] focus:border-transparent transition-all duration-200 resize-none min-h-[300px]"
                          />
                        </div>
                        <div className="flex gap-3">
                          <button
                            onClick={handleAddTemplate}
                            disabled={!newTemplate.title.trim() || !newTemplate.prompt.trim()}
                            className="px-6 py-3 bg-gradient-to-r from-[#da7756] to-[#bd5d3a] hover:from-[#bd5d3a] hover:to-[#a8462a] disabled:from-gray-400 disabled:to-gray-500 text-white rounded-lg font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            Create Template
                          </button>
                          <button
                    ...
                    <div className="flex flex-col h-full">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-semibold text-[#3d3929] dark:text-[#f0eee5]">Edit Template</h3>
                        <button
                          onClick={handleCancelEdit}
                          className="p-2 text-[#8a8470] hover:text-[#da7756] transition-colors"
                        >
                      ...
                      <div className="space-y-4 flex-1 flex flex-col">
                        ...
                        <div className="flex gap-3">
                          <button
                            ...
                            className="px-6 py-3 bg-gradient-to-r from-[#da7756] to-[#bd5d3a] hover:from-[#bd5d3a] hover:to-[#a8462a] text-white rounded-lg font-medium transition-all duration-200"
                          >
                            Save Changes
                          </button>
                          <button
                    ...
                    <div className="flex flex-col h-full">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-semibold text-[#3d3929] dark:text-[#f0eee5]">{selectedTemplate.title}</h3>
                        <div className="flex gap-2">
                          <button
                            onClick={handleEditTemplate}
                            className="px-4 py-2 bg-gradient-to-r from-[#da7756] to-[#bd5d3a] hover:from-[#bd5d3a] hover:to-[#a8462a] text-white rounded-lg font-medium transition-all duration-200 text-sm flex items-center gap-1"
                          >
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          {!selectedTemplate.isDefault && (
                            <button
                              ...
                              className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg font-medium transition-all duration-200 text-sm flex items-center gap-1"
                            >
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                              </svg>
                      ...
                      <div className="space-y-4 flex-1 flex flex-col">
                        {selectedTemplate.description && (
                          <div>
                            <label className="block text-sm font-semibold text-[#3d3929] dark:text-[#f0eee5] mb-2">Description</label>
                            <p className="text-sm text-[#6b6651] dark:text-[#c7c1a8] bg-[#eeece2] dark:bg-[#2a2520] p-3 rounded-lg border border-[#d4d0c1]/50 dark:border-[#4a453c]/50">
                              {selectedTemplate.description}
                            </p>
                          </div>
                        )}
                        <div className="flex-1 flex flex-col">
                          <label className="block text-sm font-semibold text-[#3d3929] dark:text-[#f0eee5] mb-2">Prompt</label>
                          <div className="flex-1 bg-[#eeece2] dark:bg-[#2a2520] p-4 rounded-lg border border-[#d4d0c1]/50 dark:border-[#4a453c]/50 overflow-y-auto">
                            <pre className="text-sm text-[#6b6651] dark:text-[#c7c1a8] whitespace-pre-wrap font-sans leading-relaxed">
                              {selectedTemplate.prompt}
                            </pre>
                          </div>
                        </div>
                        {selectedTemplate.isDefault && (
                          <div className="flex items-center gap-2 text-sm text-[#8a8470] dark:text-[#c7c1a8]">
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                              <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                            </svg>
                            This is a default template and cannot be deleted.
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-center">
                      <div className="text-center text-[#8a8470] dark:text-[#c7c1a8]">
